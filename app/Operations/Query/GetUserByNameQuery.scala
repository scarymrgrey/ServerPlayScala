...
case class UserResponse(username: String, online: Boolean, wins: Int, losses: Int)

final case class GetUserByNameQuery(login: String) extends QueryBase[Option[UserResponse]] {
  override def ExecuteResult(): Option[UserResponse] = {
    Repository.GetSome[User](MongoDBObject("login" -> login)).headOption match {
      case Some(usr) =>
       ...
       // val gamesWin = Repository.GetSome[Game](MongoDBObject("won" -> login,"finished"-> true)).count
        Option(UserResponse(usr.login, active.getOrElse(false), 0, 0))
